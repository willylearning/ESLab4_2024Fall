...
#if _LAB4_ONLY  // add Sampling Freq Characteristic to Hardware Service
...
tBleStatus Add_HWServW2ST_Service(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
    ...
    // 添加采样频率特征（可写属性，CHAR_PROP_WRITE）
    COPY_SAMPLING_FREQ_CHAR_UUID(uuid);  // 采样频率的 UUID
    BLUENRG_memcpy(&char_uuid.Char_UUID_128, uuid, 16);
  #if 0  // 2-byte write data are all zero, why? 
    ret = aci_gatt_add_char(HWServW2STHandle, UUID_TYPE_128, char_uuid.Char_UUID_128,
                            2, // 采样频率特征大小（单位字节）
                            CHAR_PROP_WRITE|CHAR_PROP_WRITE_WITHOUT_RESP, // 可写属性
                            ATTR_PERMISSION_NONE,
                            GATT_NOTIFY_ATTRIBUTE_WRITE, //
                            16, 1, &SamplingFreqCharHandle);
  #else // 2-byte write data request from client, need to call "aci_gatt_write_response()" for acknowledge
    ret = aci_gatt_add_char(HWServW2STHandle, UUID_TYPE_128, char_uuid.Char_UUID_128,
                            2, // 采样频率特征大小（单位字节）
                            CHAR_PROP_WRITE, //CHAR_PROP_WRITE_WITHOUT_RESP, // 可写属性
                            ATTR_PERMISSION_NONE,
...
#if 0
void Write_Request_CB(evt_gatt_write_permit_req *req)
{
    // Handle the sampling frequency write request
    if (req->attr_handle == SamplingFreqCharHandle + 1) {
        uint16_t freq;
        // Assuming data is sent as 2 bytes
        memcpy(&freq, req->data, sizeof(freq));
        printf("freq: %04x, %x, %02x %02x\n", freq, req->data_length, req->data[0], req->data[1]);
...
void Write_Request_CB(uint16_t handle, uint8_t* data, uint8_t length) {
    tBleStatus ret;
        // 检查写请求是否针对采样频率特征
    if(handle == SamplingFreqCharHandle + 1)
    {
        // 读取客户端写入的新采样频率
        new_sampling_freq = ((uint16_t)data[1] << 8) + data[0]; // 假设采样频率为2字节
        if (new_sampling_freq==0)
          new_sampling_freq = 1;
        PRINTF("New Sampling Frequency set: %d Hz\r\n", new_sampling_freq);
        osDelay(3000);  
    }
    ...
    // {
    //     // 允许客户端写入完成的响应
    //     ret = aci_gatt_write_response(connection_handle, handle, 0, 0, BLE_STATUS_SUCCESS);
    //     if (ret != BLE_STATUS_SUCCESS)
    //     {
