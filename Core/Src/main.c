...
void StartTaskBLE(void *argument)
{
  ...
  {
    printf("tb\n");
	  // check BLE events and deal with them
	  MX_BlueNRG_MS_Process();
    osDelay(100);
  }
  /* USER CODE END 5 */
...
void StartTaskACC(void *argument)
{
  ...
  for(;;)
  {
    ...
    if ((curTime - prevTime) >= (uint32_t)1000/new_sampling_freq) {
      prevTime = curTime;
      BSP_ACCELERO_AccGetXYZ(pDataXYZ);
      bAccUpdate = 1;
      
      printf("%d X: %d, Y: %d, Z: %d\n", curTime, pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);
      osDelay(100); 
    } 
  }
